...
func _run() -> void:
	...
	game._update_ui()
	_expect(game.combo_labels[0].text.is_empty(), "P1 combo label must clear after recovery")
	...
	game.fighters[0].combo_received = 0
	game._update_ui()
	_expect(game.combo_labels[1].text.is_empty(), "P2 combo label must clear after recovery")
